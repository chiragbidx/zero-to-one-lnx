...
- `app/auth/verify-email/[token]/page.tsx`: Server component that consumes an email verification token and marks the user as verified.
...
- `/auth/verify-email/[token]` → `app/auth/verify-email/[token]/page.tsx`
  ...
  - Server-only (no client component); redirects to `/dashboard/settings` on success or `/auth` on failure.
